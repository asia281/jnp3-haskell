...
mapSupply2 f (S ga) (S gb) = S gc 
  where 
    gc s = (f (fst (ga s)) (fst s'), snd s')
      where s'  = (gb (snd (ga s)))


bindSupply :: Supply s a -> (a->Supply s b) -> Supply s b
bindSupply (S fa) f = S gb
  where 
    gb s = (fb s')
      where (a, s') = fa s
            (S fb) = f a
...
labelTree t = evalSupply (go t) nats
  where
    ...
    go t = S (labels t)
      where 
        -- Label Leaf a with x, pass the rest: xs
        labels (Leaf a) (x :> xs) = (Leaf x, xs)        
        -- Label subtree a with s, then label subtree b with s', pass the rest: s''    
        labels (Branch a b) s     = ((Branch a' b'), s'') 
          where
            (a', s') = labels a s
            (b', s'') = labels b s'
-- | Example:
